...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		if (!file) {
			std::cerr << "ERROR: An input file cannot be opened.\n";
		...
		do {
			...
			if (bookline.c_str()[0] != '#' && bookline.size() > 0) {
		...
		do {
			...
			if (bookline.c_str()[0] != '#' && bookline.size() > 0) {
				if (cnt < 7) {
					...
					// Handle the case where there are more books in the file than the array can hold.
					std::cerr << "WARNING: Too many books in the input file. Some books were not loaded.\n";
					break;
				}
			}
		} while (file);
	...
	auto exchange = [usdToCadRate, gbpToCadRate](sdds::Book& book) {
		if (book.country() == "US") {
	...
	if (argc > 2) {
		...
		do {
			...
			if (bookline.c_str()[0] != '#' && bookline.size() > 0) {
				if (cnt < 5) {
					movies[cnt] =  sdds::Movie(bookline);
					++cnt;
				}
				else {
					// Handle the case where there are more books in the file than the array can hold.
					std::cerr << "WARNING: Too many movies in the input file. Some movies were not loaded.\n";
					break;
				}
			}
		} while (file);
