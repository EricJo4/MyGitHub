...
function kebab(value) {
  result = value.trim().toUpperCase().replace(/[ \t\.]{1,}/g, '-');
  return result;
}
...
function createImg(src, alt, width) {
  src = src.trim();
  alt = alt.trim();
  let result = `<img src="${src}" alt="${alt}">`;
...
function parseDateString(value) {
  ...
  if(isNaN(year)||isNaN(month)||isNaN(day)){
    throw new Error("One or more date components are not valid.");
  }
    ...
    if(year % 4 === 0){
      if(day < 1 || day > 29){
        throw new Error("Invalid Day. Day must be between 1 and 29");
      }
      else if(day < 1 || day >28){
        throw new Error("Invalid Day. Day must be between 1 and 28");
      }
    }
  }
  else if(month % 2 === 0){
...
function formatCoord(lat, lng, includeBrackets) {
  ...
  else if (validateCoord(lat,lng) && includeBrackets){
    return `[${lat},${lng}]`;
  }
    throw new Error("One or more values are invalid");
}
...
function formatCoords(...values) {
  ...
  for(let i = 0; i < values.length; i+=2){
    ...
    if(validateCoord(lat, lng)){
      coords.push(formatCoord(lat,lng,true));
    }
    else{
      throw new Error("One or more values are invalid");
    }
...
function generateLicenseLink(licenseCode) {
  ...
  if(!codes[licenseCode]){
    return `<a href="https://choosealicense.com/no-permission/">All Rights Reserved</a>`
  }
  return `<a href="https://creativecommons.org/licenses/${licenseCode.substr(3).toLowerCase()}/4.0/">${codes[licenseCode]}</a>`
}
...
function pureBool(value) {
  if(value === null || value === undefined){
    ...
    if(value === 'yes' || value === 'y' || value === 't'|| value === 'true')
      return true;
    else if(value === 'no' || value === 'n' || value === 'f' || value === 'false') {
      return false;
    }
  }
...
function all(...values) {
  let result = values.map(pureBool);
...
function buildUrl(q, perPage, order, license, lat, lng) {
  let accpetedLicense = ['none', 'any', 'cc-by', 'cc-by-nc', 'cc-by-sa', 'cc-by-nd', 'cc-by-nc-sa', 'cc-by-nc-nd'];
  if(q&&Number.isInteger(perPage) && perPage > 0 && perPage <= 200 && (order === 'asc'||order === 'desc') && accpetedLicense.includes(license) && validateCoord(lat, lng)){
    return `https://api.inaturalist.org/v1/observations?q=${q}&per_page=${perPage}&order=${order}&license=${license}&lat=${lat}&lng=${lng}`;
  }
