...
function observationsByGeoPrivacy(data, geoPrivacy) {
  ...
  // String values should be lower-cased, but ignore null value
  if (typeof geoPrivacy === 'string') {
    geoPrivacy = geoPrivacy.toLowerCase();
  }

  for (let element of results) {
    if (element.geoprivacy === geoPrivacy) {
...
function transformObservations(data) {
  ...
  results.forEach((element) => newArray.push(transformObservation(element)));
  return newArray;
}
...
function transformObservations2(data) {
  const { results } = data;
  return results.map(transformObservation);
}
...
function getObservationsById(data, ...ids) {
  ...
  function getId(id) {
    const observation = results.find((element) => element.id === id);
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  const { equal, min, max } = options;

  let filteringFn;

  if (equal) {
    filteringFn = (o) => o.positional_accuracy === equal;
  } else if (min && max) {
    filteringFn = (o) => o.positional_accuracy >= min && o.positional_accuracy <= max;
  } else if (min) {
    filteringFn = (o) => o.positional_accuracy >= min;
  } else if (max) {
    filteringFn = (o) => o.positional_accuracy <= max;
  } else {
    // Include all values
    filteringFn = (o) => o;
  }

  return data.results.filter(filteringFn);
}
...
function getTaxonPhotos(data) {
  ...
  for (let element of results) {
    try {
      const url = element.taxon.default_photo.square_url;
      photo.push({
        square: url,
        small: url.replace('square.jpg', 'small.jpg'),
        medium: url.replace('square.jpg', 'medium.jpg'),
        large: url.replace('square.jpg', 'large.jpg'),
        original: url.replace('square.jpg', 'original.jpg')
      });
    } catch (err) {
      continue;
    }
...
function getUserStats(data) {
  ...
  results.forEach((element) => {
    const { user } = element;
    totals.observations += user.observations_count;
    totals.identifications += user.identifications_count;
    totals.species += user.species_count;
  });
  ...
  return {
    ...
    totals,
    averages: {
      observations: totals.observations / count,
      identifications: totals.identifications / count,
      species: totals.species / count
    }
  };
}
