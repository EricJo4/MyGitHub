...
class Slider:
    def __init__(self, x, y, width, min_value, max_value, initial_value):
        ...
        self.dragging = False
        self.slider_x = self.x + (self.value - self.min_value) / (self.max_value - self.min_value) * self.width

    def draw(self, window):
        pygame.draw.rect(window, BLACK, (self.x, self.y, self.width, 10))
        pygame.draw.circle(window, BLACK, (int(self.slider_x), self.y + 5), 8)
        font = pygame.font.Font(None, 24)
        text = font.render(f"{int(self.value)}", 1, BLACK)
        window.blit(text, (self.slider_x - 10, self.y + 15))

    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            if pygame.Rect(self.slider_x - 8, self.y - 8, 16, 16).collidepoint(event.pos):
                self.dragging = True
        elif event.type == pygame.MOUSEBUTTONUP:
            self.dragging = False
        elif event.type == pygame.MOUSEMOTION:
            if self.dragging:
                self.slider_x = min(max(self.x, event.pos[0]), self.x + self.width)
                self.value = self.min_value + (self.slider_x - self.x) / self.width * (self.max_value - self.min_value)

    def get_value(self):
        return int(self.value)

# (improvement: Undo) Additional Pygame button for undo
class Button:
...
class Board:
    def __init__(self,width,height, p1_sprites, p2_sprites):
        ...
        self.turn = 0
        self.move_stack = []  # (improvement: undo) Stack to store the game state for undo functionality

    # (improvement: undo)Save the current game state to the stack
    def save_state(self):
        self.move_stack.append((self.get_board(), self.turn))

    # (improvement: undo)Restore the last saved state from the stack
    def undo_move(self):
        if self.move_stack:
            last_state, self.turn = self.move_stack.pop()
            self.set(last_state)
...
#(improvement: undo) undo button location
undo_button = Button("Undo", 900, 200, 100, 50)

#(improvement: Bot Smartness) Create sliders for both players
player1_slider = Slider(900, 300, 200, 1, 5, 1)
player2_slider = Slider(900, 330, 200, 1, 5, 1)
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            player1_slider.handle_event(event)  # (improvment: Bot Smartness)
            player2_slider.handle_event(event)  # (improvment: Bot Smartness)
            ...
            # (improvement: undo) undo button event
            if undo_button.handle_event(event) and not has_winner:
    ...
    if not has_winner:
        if overflowing:
            ...
            if choice[current_player] == 1:
                tree_height = player1_slider.get_value() if current_player == 0 else player2_slider.get_value()  # (improvement: Bot Smartness)
                bots[current_player].set_tree_height(tree_height)  # (improvement: Bot Smartness) Modify bots to set tree height
            ...
            if make_move:
                board.save_state()  # (improvement: undo) Save state before making the move
    ...
    #(improvement: undo) undo button
    undo_button.draw(window)
    #(improvement: Bot Smartness) Bot Smartness Sliders
    player1_slider.draw(window)
    player2_slider.draw(window)
